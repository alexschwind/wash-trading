...
detect_and_label_wash_trades_for_scc_using_multiple_passes <- function(trades, relevant_scc, window_sizes_in_seconds, window_start, ether = TRUE, margin,
  ...
  for (window_size_index in 1:length(window_sizes_in_seconds)) {
    ...
    for (scc.id.index in 1:length(relevant_scc)) {
      ...
      temp_trades$group_token <- temp_trades$token
      temp_trades$group_window <- temp_trades$window
      temp_trades_per_token_and_window <- split(temp_trades, list(temp_trades$token, temp_trades$window), drop = TRUE)

      # input zu der funktion ist gleich, weil ich die function calls gesehen habe. die funktionen haben immer die gleichen transaction inputs
      ...
      fwrite(checked_trades, paste0("r_function_output",scc.id.index,window_size_index,".csv"))
  ...
  fwrite(trades, "final_trades.csv")
  if (save) {
    # remove file type if given
    filename <- gsub("\\..*", "", filename)
