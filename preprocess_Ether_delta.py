...
def main(argv):
    ...
    EtherDelta_tokens = pd.DataFrame(pd.unique(pd.concat([trades['tokenGet'], trades['tokenGive']], axis=0)),
                                     columns=['address'])
    ...
    if (len(missing_EtherDelta_tokens) > 0):
        print("Warning:", len(missing_EtherDelta_tokens),
              "tokens in EtherDelta data have no decimal information in respective file. " +
    ...
    print(
        "Info: converted column 'amountGet' to 'amountBoughtReal' as the trade amount of 'tokenGet', according to token decimals.")
    ...
    print(
        "Info: converted column 'amountGive' to 'amountSoldReal' as the trade amount of 'tokenGive', according to token decimals.")
